...
@section('content') 
    <div class="row">
        ...
        <div class="col-12 col-md-8 text-md-right mb-4">
            ...
            @endif
            @if($invoice->paid_status != 'PAID' and $invoice->company->isMollieActive())
                <a href="{{ route('customer_portal.invoices.mollie.payment', ['customer' => $currentCustomer->uid, 'invoice' => $invoice->uid]) }}" class="btn btn-primary">
                    <i class="material-icons">payment</i> 
                    {{ __('messages.pay_with_mollie') }}
                </a>
            @endif
        </div>
        ...
        <div class="col-12">
            @if($invoice->paid_status == 'UNPAID')
                <div class="alert alert-soft-danger d-flex align-items-center" role="alert">
                    ...
                    <div class="text-body"><strong>{{ __('messages.status') }} : </strong> {{ $invoice->paid_status }}</div>
                </div>
